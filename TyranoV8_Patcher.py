...
class App(tk.Tk):
    ...
    def _file_entry(self, parent, label, is_dir=False, ext=None):
        ...
        self._setup_drag_drop(entry, var)
    ...
    def _setup_drag_drop(self, widget, var):
        try:
    ...
    def _init_save_manager_ui(self):
        ...
        self.tree = ttk.Treeview(tree_f, columns=('name'), show='headings', selectmode='browse')
        ...
        ttk.Checkbutton(btn_f, text=T('chk_zip'), variable=self.var_zip).pack(fill='x', pady=5)
    ...
    def scan_saves(self):
        ...
        if found:
            ...
            for dn, fp, is_zip in sorted(backups, reverse=True):
                ...
                tag = "[ZIP] " if is_zip else "[DIR] "
                self.tree.insert('', tk.END, iid=iid, values=(tag + dn,))
    ...
    def _add_to_list(self, list_ref, filename, fullpath, is_zip):
        try:
            ...
            # 尝试格式化时间戳
            if len(ts) >= 15:
                 display_name = f"{ts[:8]}-{ts[9:15]}" # 简易格式
                 # 更好看一点: YYYY-MM-DD HH:MM:SS
                 if len(ts) == 15:
                     display_name = f"{ts[:4]}-{ts[4:6]}-{ts[6:8]} {ts[9:11]}:{ts[11:13]}:{ts[13:]}"
            else:
        ...
        def _w():
            try:
                if os.path.isfile(src):
                    os.remove(src) # 删除 ZIP
                else:
                    shutil.rmtree(src, onerror=self.core.remove_readonly) # 删除文件夹
    ...
    def _init_tools_ui(self):
        ...
        self.var_plat = tk.StringVar(value='win')
        ttk.Radiobutton(f_plat, text=T('rad_win'), variable=self.var_plat, value='win').pack(side='left')
        ttk.Radiobutton(f_plat, text=T('rad_mac_linux'), variable=self.var_plat, value='nix').pack(side='left', padx=5)
    ...
    def _auto_scan_asar(self):
        ...
        for c in cands:
            p = os.path.abspath(c)
            if os.path.exists(p):
                self.var_ext_src.set(p); self.log(f"Found: {p}"); return
        self.log("Not found.")
    ...
    def _tool_extract(self):
        ...
        def _t():
            ...
            finally: self.toggle_progress(False)
    ...
    def _sync_extracted_path(self):
        ...
        if self.last_extracted_path and os.path.exists(self.last_extracted_path):
            ...
            # 尝试寻找最新的 _Unpacked 目录
            unpacked_root = os.path.join(os.getcwd(), "_Unpacked")
            if os.path.exists(unpacked_root):
                dirs = [os.path.join(unpacked_root, d) for d in os.listdir(unpacked_root)]
    ...
    def _tool_pack(self):
        ...
        out_name = os.path.basename(src).replace("_extracted", "") + ".new.asar"
        if not out_name.endswith(".asar"): out_name += ".asar"
        ...
        if os.path.exists(out_path):
            ...
            try:
                ...
                try: shutil.rmtree(out_path, onerror=self.core.remove_readonly)
                except: pass
